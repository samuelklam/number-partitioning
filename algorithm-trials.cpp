...
void run_rand_algos(vector<int64_t> A, int num_iterations, ofstream *file_arr[7]) {
    ...
    // Pre-Partioned Random Solution and copy of A to use for pre-partitioned algos
    vector<int64_t> P = get_rand_prepart(A.size()), A_copy = A;
    ...
    start = clock();
    read_to_file(*file_arr[5], "Hill-Climbing Pre-Part", hill_climbing_prepart(A_copy, P, num_iterations), start, num_iterations);
    
    // Pre-Partioned Simulated Annealing
    start = clock();
    read_to_file(*file_arr[6], "Simulated-Annealing Pre-Part", simulated_annealing_prepart(A_copy, P, num_iterations), start, num_iterations);
}
